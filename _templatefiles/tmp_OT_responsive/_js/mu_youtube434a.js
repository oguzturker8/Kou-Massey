...
function onInLinePlayerStateChange(event){		
	...
	else if (!bPopUpLoaded)
	{
		...
		thisVideo = $j(".flex-active-slide .mu_slideVideo").html();						
		//thisVideo = $j(".mu_slideVideo").html();
		...
		if (thisVideo.length && videoPaused) {
			...
			// Pop up video
			$j("#muPopupVideoDialog").dialog("option", "width", iDialogWidth);
			$j("#muPopupVideoDialog").dialog("option", "alsoResize", "#popupVideo");
			$j("#muPopupVideoDialog").data("myVideo", thisVideo).dialog("open");
			iPopUpLoaded = 1;						
		}				
...
//$j(window).load(function() {
	...
	$j('body').append('<div id="muPopupVideoDialog" style="display:none;"><\/div>');
